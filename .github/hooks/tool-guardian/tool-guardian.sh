...
if command -v jq &>/dev/null; then
  ...
  TOOL_INPUT=$(printf '%s' "$INPUT" | jq -c '.tool_input // .toolInput // .' 2>/dev/null || printf '%s' "$INPUT")
fi

# Fallback: extract with grep/sed if jq unavailable or fields empty
if [[ -z "$TOOL_NAME" ]]; then
  TOOL_NAME=$(printf '%s' "$INPUT" | sed -nE 's/.*"(tool_name|toolName)"[[:space:]]*:[[:space:]]*"([^"]*)".*/\2/p' | head -1)
fi
...
if [[ $THREAT_COUNT -gt 0 ]]; then
  echo "" >&2
  echo "Tool Guardian: $THREAT_COUNT threat(s) detected in '$TOOL_NAME' invocation" >&2
  echo "" >&2
  ...
  if [[ "$MODE" == "block" ]]; then
    ...
    printf '%s\n' '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Tool Guardian blocked a high-risk operation. Review the command or use TOOL_GUARD_ALLOWLIST for an approved exception."}}'
    exit 2
  else
    echo "Threats logged in warn mode. Set GUARD_MODE=block to prevent dangerous operations." >&2
    printf '%s\n' '{"continue":true,"systemMessage":"Tool Guardian detected a high-risk operation in warn mode."}'
  fi
else
  # Log clean result
